...
Stats *stat(const Array *array){
    ...
    stats->modFreq=0;
    int freq[array->size] = {0};
    int nModes=0;
    //Use array numbers as indexes and increment
    for(int i=0; i<array->size;i++) freq[array->data[i]]++;
    //Find the max mode frequency 
    for(int i=0; i<array->size;i++) if(freq[i]>stats->modFreq)stats->modFreq=freq[i];
    //Find the Number of Modes
    for(int i=0; i<array->size; i++)if(freq[i]==stats->modFreq && stats->modFreq > 1)nModes++; 

    stats->mode->size=nModes;
    //Create the mode array
    if(nModes!=0)stats->mode->data=new int[nModes];
    //Write the Modes into the mode array
    if(nModes!=0){
        for(int i=0; i<array->size; i++){
            if(freq[i]==stats->modFreq)stats->mode->data[i]=i;
        }
    }


    //Get the Median
    if(array->size%2==0)stats->median=(array->data[array->size/2]+array->data[(array->size-1)/2])/2.0;
    else stats->median=array->data[array->size/2];

    //Get the Average
    stats->avg=0;
    for(int i=0; i<array->size; i++)stats->avg+=array->data[i];
