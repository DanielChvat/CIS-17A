...
void filArry(string, char *, const int);
    ...
    do{
        ...
        if(money>0){
            ...
            if(crntIdx<NCARDS){
                ...
                //Fill Cards Array
                filArry(cardSel,card.crd,NCARDS);
                getline(suits,suitSel);
                //Fill suits array
                filArry(suitSel,card.cSut,NCARDS);
                //Fill cards and Suits Table to be printed later
                //Display a message of the card and its suit that the player recieved
                pCards[crntIdx]=getCard(card.crd, crntIdx);
                pSuits[crntIdx]=getSuit(card.cSut,crntIdx);
...
void filArry(string text, char *array, const int NCARDS){
    for(int i=0; i<NCARDS;i++){
        array[i]=text[i];
    }
}
