...
int main(){
    ...
    card.cSut = new char[NCARDS];
    card.crd = new char[NCARDS];
    ...
    Player *player = init(money, strtMny);
    player->pCards=new string[NCARDS];
    player->pSuits=new string[NCARDS];
    //Initialize Variables
    crntIdx=0;
    file->fileNm1="Cards.dat";
    file->fileNm2="Suits.dat";
    cards.open(file->fileNm1,ios::out);
    suits.open(file->fileNm2,ios::out);
    bet=0;
    ...
    do{
        ...
        cards.open(file->fileNm1,ios::in);
        suits.open(file->fileNm2,ios::in);
        cout<<endl;
        ...
        if(money>0){
            ...
            if(crntIdx<NCARDS){
                getline(cards,card.cardSel);
                //Fill Cards Array
                filCrd(card, NCARDS);
                getline(suits,card.suitSel);
                //Fill suits array
                filSut(card, NCARDS);
                //Fill cards and Suits Table to be printed later
                //Display a message of the card and its suit that the player recieved
                player->pCards[crntIdx]=getCard(card.crd, crntIdx);
                player->pSuits[crntIdx]=getSuit(card.cSut,crntIdx);
                //Calculate the current value of all the players cards         
                pVal=pVal=getVal(card.crd,crntIdx,pVal);
                pVals[crntIdx]=pVal;
                pVal=CrdVal(pVals,NCARDS);
                cout<<endl;
                cout<<"You Recieved: "<<player->pCards[crntIdx]<<" of "<<player->pSuits[crntIdx]<<endl;
                cout<<endl;
                ...
                do{
                    if(hit=='Y'||hit=='y'){
                        if(crntIdx<52){
                            ...
                            player->pSuits[crntIdx]=getSuit(card.cSut,crntIdx);
                            //Calculate the current value of all the players cards         
                            pVal=getVal(card.crd,crntIdx,pVal);
                            pVals[crntIdx]=pVal;
                            pVal=CrdVal(pVals,NCARDS);
                            cout<<endl;
    ...
    cout<<endl<<endl<<"Thanks For Playing "<<player->name<<"!"<<endl;
    destroy(file, card, player);
    return 0;
}
...
Player *init(float &money, float &strtMny){
    money=0;
...
void destroy(File *file, Card card, Player *player){
