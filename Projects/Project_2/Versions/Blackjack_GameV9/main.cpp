...
int main(){
    ...
    file[0]->getFile()->write(card.getCrd(), sizeof(char)*card.getNCrd());
    file[1]->getFile()->write(card.getSut(), sizeof(char)*card.getNCrd());
    ...
    file[1]->getFile()->close();
    file[2]->getFile()->close();
    stats(file[2], card.getNCrd(), card.getCrd(), card.getSut());
